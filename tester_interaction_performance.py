...
def update_interaction_graph(risk_owner_hazard, timehorizon, scenarios, performance_metric, interactions_of_interest, interaction_plot, plot_type):
    ...
    if interaction_plot == 'graph':
        ...
        print(interactions_of_interest.split('&'))
        interactions_title = ', '.join([ROH_DICT_INV[interaction] for interaction in interactions_of_interest.split('&')])
        print(interactions_title)
        ...
        performance_df_of_interest_interactions = pd.read_csv(
            f'{DIRECTORY_PATH_PERFORMANCE}/interactions/performance_{risk_owner_hazard}_combi_{interactions_of_interest}.csv')
        ...
        if plot_type == 'PCP':
            return Parallel_Coordinates_Plot(df=filtered_df, risk_owner_hazard=risk_owner_hazard,
                                             df_interaction=filtered_df_interactions, figure_title=figure_title) # Show graph, hide image
    ...
    #
    #     figure_identifier = f'{risk_owner_hazard}_{scenarios_str}_{performance_metric}_{interactions_of_interest}.png'
    #
...
for plot_type in ['PCP', 'StackedBar', 'Heatmap']:
    for risk_owner_hazard in ROH_DICT_INV:
        interaction_combinations = generate_combinations(ROH_LIST, risk_owner_hazard)
            ...
            for timehorizon in TIMEHORIZONS_INV:
                for scenarios in SCENARIO_OPTIONS:
                    ...
                    for interaction in interaction_combinations:
                        print(interaction)
                        fig = update_interaction_graph(risk_owner_hazard, timehorizon, scenarios, performance_metric, interaction, 'graph', plot_type)
                        pathlib.Path(f'Dashboard_v1/assets/figures/{plot_type}/{risk_owner_hazard}/{interaction}/').mkdir(parents=True, exist_ok=True)
                        fig.write_json(f'Dashboard_v1/assets/figures/{plot_type}/{risk_owner_hazard}/{interaction}/plot_{timehorizon}_{scenario_str}_{performance_metric}.json')